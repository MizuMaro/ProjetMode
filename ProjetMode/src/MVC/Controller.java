		...
		v.getFenetre().addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyChar() == 'r') {
					...
					setPositionOiseauC2((int) m.getPositionOiseau().getX(),
							(int) m.getPositionOiseau().getY());
					repaint();
		...
		v.getFenetre().addMouseMotionListener(new MouseInputAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				if (!m.getOiseau().getVole()) {
					...
					/*
					 * double aa = m.getOiseau().distance(m.getOiseau().getC().x
					 ...
					 * 
					 * double cc =
					 * m.getOiseau().distance(Constantes.getInstance(
					 * ).COORDONNEES_ORIGINE.x +
					 * Constantes.getInstance().TAILLE_OISEAU / 2,
					 * Constantes.getInstance().COORDONNEES_ORIGINE.y +
					 * Constantes.getInstance().TAILLE_OISEAU / 2,
					 * Constantes.getInstance().COORDONNEES_ORIGINE.x-100 +
					...
					if (e.getX() < Constantes.getInstance().COORDONNEES_ORIGINE.x
							+ 20 + Constantes.getInstance().RAYON_DEPART
							&& e.getX() > Constantes.getInstance().COORDONNEES_ORIGINE.x
									+ 20
									- Constantes.getInstance().RAYON_DEPART
							&& e.getY() < Constantes.getInstance().COORDONNEES_ORIGINE.y
									+ 2
									* 20
									+ Constantes.getInstance().RAYON_DEPART
							&& e.getY() > Constantes.getInstance().COORDONNEES_ORIGINE.y
							...
							&& m.getOiseau()
									.distance(
											...
											m.getPositionOiseau().getY()
													+ Constantes.getInstance().TAILLE_OISEAU
													/ 2) < Constantes
									.getInstance().RAYON_DEPART) {

						// Calcul de l'angle à Remi

						double angle = (double) Math.toDegrees(Math.atan2(m
								.getOiseau().getC().getY() - 350, m.getOiseau()
								.getC().getX() - 150));
						if (angle < 0) {
							angle += 360;
						}
						...
						setPositionOiseauC2(m.getPositionOiseau().x + 50,
						...
						// m.getOiseau().setAngle((bb*bb + cc*cc - aa*aa) /
						// (2*bb*cc));
				...
				if (m.getDrag() && !m.getVol()) {
					...
					if (Model.physique == false) {
						...
						if (posLanY >= 350 && posLanX <= 150) {
							...
							double c = 100;
							double calcul = ((posLanX / 1000000) * 6 + 0.00030)
									* Math.pow(m.getPositionOiseau().getX(), 2)
									+ (((posLanY - 350) / 7) - 1.05 - (1.02 * ((posLanY - 350) / 7)))
									* m.getPositionOiseau().getX() + c;
							while (Math.abs(calcul - posLanY) > 3
									&& calcul < 1000) {
								c = c + 2;
								calcul = ((posLanX / 1000000) * 6 + 0.00030)
										* Math.pow(
												m.getPositionOiseau().getX(), 2)
										+ (((posLanY - 350) / 7) - 1.05 - (1.02 * ((posLanY - 350) / 7)))
										* m.getPositionOiseau().getX() + c;
							}
						...
						phy.trajectoireParabole(m.getOiseau(), vitesse, rad,
								m.getAffichage(), m);

						if (m.getOiseau().getScore() != 0
								&& m.getOiseau().getScore() == m
										.getCptObstacles()) {
