...
/**
 * La classe Menu permet d'avoir un menu graphique au lancement du programme,
 * avec une selection d'options possibles pour l'utilisateur.
 * 
 * @author Rémy
 *
 */
...
public class Menu extends JPanel {
	...
	public Menu(final Model m, final FreeModel model, FreeController control,
		...
		this.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent e) {
				if (e.getX() > 700 && e.getX() < 900 && e.getY() > 50
						&& e.getY() < 150) {
					jouer_b = true;
					quitter_b = false;
					free_b = false;
				} else if (e.getX() > 700 && e.getX() < 975 && e.getY() > 250
						&& e.getY() < 350) {
					quitter_b = true;
					jouer_b = false;
					free_b = false;
				} else if (e.getX() > 700 && e.getX() < 1000 && e.getY() > 150
						&& e.getY() < 250) {
					free_b = true;
					quitter_b = false;
					jouer_b = false;
	...
	public void paintComponent(Graphics g) {
		...
		g.drawImage(Images.getInstance().SKY, 0, 0,
