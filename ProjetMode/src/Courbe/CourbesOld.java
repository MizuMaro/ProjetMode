...
/**
 * Ancienne classe qui servait a lancer les oiseaux. Celle-ci n'est plus
 * utilisee car elle n'utilisait pas de Timer.
 * 
 * @author Rémy
 *
 */
public class CourbesOld {
	...
	public void courbe(double a, double b, double c, Oiseau o,
			Affichage affichage, ArrayList<Obstacle> obstacles) {
		...
		while (x <= 800 * 7 && !touch && timeLancement < timeFin) {
			...
			limites = (o.getC().x > Constantes.getInstance().TAILLE_ECRAN[0]
					- Constantes.getInstance().TAILLE_OISEAU || o.getC().x < 0);
			...
			for (Obstacle ob : obstacles) {
				...
				if (ob.isActif()
						&& o.distance(o.getC().getX(), o.getC().getY(), ob
								.getC().getX(), ob.getC().getY()) <= (Constantes
	...
	public void courbeCubique(double a, double b, double c, Oiseau o,
			Affichage affichage, ArrayList<Obstacle> obstacles) {
		...
		while (x <= 800 * 7 && !touch && timeLancement < timeFin) {
			...
			// collision avec le sol
			sol = (o.getC().getY() >= Constantes.getInstance().HAUTEUR_SOL
					- Constantes.getInstance().TAILLE_OISEAU);
			// sortie de l'ecran
			limites = (o.getC().x > Constantes.getInstance().TAILLE_ECRAN[0]
					- Constantes.getInstance().TAILLE_OISEAU || o.getC().x < 0);
			...
			// pour tangente
			x2 = x + 50;
			y2 = (int) ((int) (a * 3 * Math.pow(x2, 2) + b) * (x2 - a) + (a
					* Math.pow(x2, 3) + b * x2 + c));
			// courbe ax2+bx+c
